...
Each recorded demonstration is regenerated by its own script, which drives
the versioned tape itself. Running a tape directly overwrites the verified
media with a broken take, because the workspace its commands expect is gone.
The [release checklist](docs/how-to/release.md) states the commands.
