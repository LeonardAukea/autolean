"""Supplementary CLI workflows with stable compatibility names."""
...
from autolean.llm import BACKEND_NAMES, LLMError
...
def _connected_llm(*args: Any, **kwargs: Any) -> Any:
    return _root_cli()._connected_llm(*args, **kwargs)


def _accept_generated_source(*args: Any, **kwargs: Any) -> Any:
    return _root_cli()._accept_generated_source(*args, **kwargs)
...
def _run_agent(*args: Any, **kwargs: Any) -> Any:
    return _root_cli()._run_agent(*args, **kwargs)
...
def _prepare_challenge_source(
    ...
    if not path.exists():
        return _accept_generated_source(lean_root, path, rendered_source, timeout=300), False
