...
def prove(statement: str, model: str | None, max_attempts: int, program: Path) -> None:
    ...
    # Strip import lines — the file already has imports at the top.
    # (Same logic as create_verification_file.)
    import re as _re
    code_lines = [
        l for l in claim.lean_code.split("\n")
...
def verify(source: str, pages: str | None, max_cycles: int, model: str | None, program: Path) -> None:
    ...
    Uses multi-strategy extraction:
      1. ar5iv HTML (best for arXiv — preserves math)
      2. pymupdf PDF extraction (universal fallback)
      3. arXiv API abstract (minimal fallback)

    \b
    Examples:
      autolean verify https://arxiv.org/abs/1811.04311
      autolean verify paper.pdf --pages 1-5
      autolean verify 2604.07408
    """
    import re as _re
    ...
    from autolean.paper import (
        analyze_paper_structure, create_verification_file,
        extract_claims_from_html, extract_claims_via_llm,
        formalize_claim, read_paper, read_paper_text, _extract_arxiv_id,
    )

    # Step 1: Extract claims (structured HTML first, LLM fallback)
    console.print(f"[bold]Analyzing paper: {source}[/]\n")
    try:
        claims, paper_title = read_paper(source, pages=pages)
    except Exception as e:
        console.print(f"[red]Paper extraction failed: {e}[/]")
        console.print("\n[yellow]Try: --pages 1-10, or check connection.[/]")
        return

    # Step 2: Setup LLM (needed for formalization, and possibly claim extraction)
    cfg = parse_program(program)
    if model:
        profile = resolve_profile(model)
        if profile:
        ...
        llm_cfg = LLMConfig(
            ...
            num_predict=32768, timeout=600.0,
        )
    ...
    # If structured extraction found nothing, fall back to LLM
    if not claims:
        console.print(f"[bold]Falling back to LLM-based extraction...[/]")
        text, paper_title = read_paper_text(source, pages=pages)
        if text and len(text.strip()) > 100:
            claims = extract_claims_via_llm(text, llm.generate)
        else:
            console.print("[red]Could not extract text from paper.[/]")
            llm.close()
            return

    if not claims:
        console.print("[yellow]No claims extracted. Try --pages or a different model.[/]")
        llm.close()
        return

    # Step 3: Analyze and display paper structure
    structure = analyze_paper_structure(claims)
    console.print(f"\n[bold]Paper structure:[/]")
    for kind, count in sorted(structure["by_kind"].items()):
        icon = {"theorem": "T", "lemma": "L", "proposition": "P",
                "corollary": "C", "definition": "D", "remark": "R"}.get(kind, "?")
        console.print(f"  [{icon}] {kind}: {count}")
    if structure["with_proof"]:
        console.print(f"  Proofs found: {structure['with_proof']}/{structure['total_claims']}")
    console.print()

    # Show all claims
    provable = structure["provable"]
    definitions = structure["definitions"]
    console.print(f"  [cyan]{len(provable)}[/] provable claims, "
                  f"[dim]{len(definitions)} definitions, "
                  f"{len(structure['remarks'])} remarks[/]\n")
    for i, c in enumerate(claims, 1):
        kind_color = {"theorem": "bold green", "lemma": "green", "proposition": "cyan",
                      "corollary": "cyan", "definition": "yellow", "remark": "dim"}.get(c.kind, "white")
        proof_marker = " [dim](has proof)[/dim]" if c.proof_sketch else ""
        console.print(f"  {i}. [{kind_color}]{c.label}[/{kind_color}]: "
                      f"{c.statement[:90]}...{proof_marker}")

    # Step 4: Formalize provable claims (skip remarks)
    to_formalize = [c for c in claims if c.kind != "remark"]
    console.print(f"\n[bold]Formalizing {len(to_formalize)} claims...[/]")
    formalized = 0
    for c in to_formalize:
        ...
        if c.lean_code:
            ...
            console.print(f"  [yellow]SKIP[/] {c.label}")
    llm.close()

    if formalized == 0:
        console.print("[yellow]No claims could be formalized.[/]")
        return

    # Step 5: Write .lean file
    safe_title = _re.sub(r"[^a-zA-Z0-9_]", "_", paper_title.replace(" ", "_"))
    lean_root = program.parent / cfg.lean_project_path
    output_path = lean_root / "AutoLean" / f"Paper_{safe_title}.lean"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    # Only write formalized claims (skip remarks)
    create_verification_file(to_formalize, output_path, paper_title=paper_title)
    console.print(f"\n[bold green]Wrote {output_path}[/]")
    console.print(f"  {formalized} theorems ready for proving")

    # Step 6: Run agent to attempt proofs (tactic pre-search + LLM)
    if max_cycles > 0:
...
def verify_paper(
    ...
    from autolean.paper import (
        analyze_paper_structure, create_verification_file,
        extract_claims_via_llm, formalize_claim,
        read_paper, read_paper_text,
    )

    # Step 1: Extract claims (structured HTML first)
    console.print(f"[bold]Analyzing paper: {source}[/]\n")
    claims, paper_title = read_paper(source, pages=pages)
    ...
    # If structured extraction found nothing, fall back to LLM
    if not claims:
        console.print(f"[bold]Falling back to LLM extraction...[/]")
        text, paper_title = read_paper_text(source, pages=pages)
        if text and len(text.strip()) > 100:
            claims = extract_claims_via_llm(text, llm.generate)

    if not claims:
        console.print("[yellow]No claims extracted.[/]")
        llm.close()
        return
    ...
    console.print()
    for i, c in enumerate(claims, 1):
        proof_marker = " (has proof)" if c.proof_sketch else ""
        console.print(f"  {i}. [bold]{c.label}[/]: {c.statement[:100]}...{proof_marker}")
    ...
    create_verification_file(to_formalize, output, paper_title=paper_title)
    console.print(f"\n[bold green]Wrote {output}[/]")
    console.print(f"  {len([c for c in to_formalize if c.lean_code])} theorems ready")
