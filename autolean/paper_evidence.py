...
def write_paper_plan(
    ...
    *,
    ...
    if not isinstance(location, InferenceLocation):
        raise ValueError("paper planning location must be local or remote")
    ...
    record = {
        ...
        "backend": backend,
        "inference_location": location.value,
        "model": model,
...
def _coverage_claim(claim: Claim) -> dict[str, object]:
    return {
        ...
        "formalization_call": (
            claim.formalization_receipt.as_dict() if claim.formalization_receipt is not None else None
        ),
        "kind": normalize_claim_kind(claim.kind),
        "label": claim.label,
...
def write_paper_coverage(
    ...
    *,
    ...
    ledger = {
        ...
        "document_extraction": (
            document_extraction_receipt.as_dict() if document_extraction_receipt is not None else None
        ),
        "pdf_sha256": artifact.pdf_sha256,
        "lean_evidence": lean_evidence,
        "model_extraction": extraction_receipt.as_dict() if extraction_receipt is not None else None,
...
def render_verification_source(
    ...
    parts = [
        *(f"import {module}" for module in imports),
        ...
        "",
        "Lean source for extracted paper items, generated by AutoLean verify.",
        "Paper-to-Lean correspondence requires mathematical review.",
    ]
